...
# Title: generate_mplug.py
# Date: 2025/11/12 14:01:46
# Description: MARINE evaluation script for mPLUG-Owl3 
# 
...
def eval_model(args):
    ...
    put_file_to_s3(answers_file, f"s3://results/CAP6614_MARINE/answers/mplug/{os.path.basename(answers_file)}")

    report_message_to_n8n(f"Sample output: `{sample_out[0]}` -> `{sample_out[1]}`", msg_type="info")
    report_message_to_n8n(f"mPLUG evaluation done! Answers saved to `{answers_file}`", msg_type="info")
    logging.info(f"Done! Saved answers to {answers_file}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_path", type=str, default="mPLUG/mPLUG-Owl3-7B-241101")
    ...
    try:
        report_message_to_n8n(f"Starting mPLUG inference with guidance strength {args.guidance_strength} on questions from ```{args.question_file}``` to generate answers to ```{args.answer_path}```.")
        eval_model(args)
    except Exception as e:
...
f"""Exception occurred during mPLUG evaluation. TL;DR: 
```{str(e)}```.
Traceback: 
```
